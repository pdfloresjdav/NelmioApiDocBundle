...
class SwaggerFormatter implements FormatterInterface
{
    ...
    {
        $resourceList = array(
            'swagger' => (string) $this->swaggerVersion,
            'apis' => array(),
        ...
        foreach ($collection as $item) {
            ...
            foreach ($responseMap as $statusCode => $prop) {
                ...
                if (isset($statusMessages[$statusCode])) {
                    ...
                    if(array_key_exists ( 'description' , $prop['type'])){
            ...
            $responseMessages['default']['description'] = "unexpected error";

            if(!empty($apiDoc->getRoute()->getMethods())){
                $apiMethod = $apiDoc->getRoute()->getMethods();
            }
            else{
                $apiMethod = array('GET');
            }

            foreach ($apiMethod as $method) {
                $operation = array(
                    'method' => $method,
                ...
                foreach ($apiBag as $path => $operations) {
                    $apiDeclaration['paths'] = array(
                        $path => array(strtolower($method) => array('parameters' => $parameters, 'responses' => $responseMessages,))
                    );
                }
            }
            $apiDeclaration['info']['version'] = (string) $this->apiVersion;
            $apiDeclaration['info']['title'] = $apiDoc->getResourceDescription();
            $apiDeclaration['info']['description'] = $apiDoc->getDescription();
            $apiDeclaration['info']['contact'] = array('name' => 'Intellectum Tech', 'email' => 'twistter@davidson.fr');
        }
